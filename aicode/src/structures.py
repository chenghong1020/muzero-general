...
class GameHistory:
    ...
    def get_stacked_observations(self, index: int, num_stacked_observations: int) -> np.ndarray:
        ...
        # 堆叠观测帧 (通常在通道维度)
        # 假设 observation_shape 是 (C, H, W)， (H, W, C) 需要输入reshape
        
        # 假设是 (C, H, W)，则堆叠后是 (num_stacked * C, H, W)
        # 这里假设是 (C, H, W)
        # 注意：实际堆叠方式取决于网络输入要求
        
        if len(observations[0].shape) == 3:  # (C, H, W)
            # 拼接观测通道
            ...
            for i in range(num_stacked_observations):
                ...
                # 将动作值编码到平面中（使用one-hot空间编码）
                plane = np.zeros((1, H, W), dtype=np.float32)
                if 0 <= action < (H * W):  # 确保动作在有效范围内
                    row = action // W
                    col = action % W
                    plane[0, row, col] = 1.0  # 在对应位置设置1
